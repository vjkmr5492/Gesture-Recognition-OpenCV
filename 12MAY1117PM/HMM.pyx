...
cdef class HiddenMarkov:
	...
	cdef long double computeAlpha(self):
		...
		# print "SUMI", t, j, i, sumi, self.alpha[t+1,j], self.output[j, self.currentSeq[t+1]]

		probability=0
		for t in range(self.len_obSeq):
			probability+=log(self.scaleFactor[t])
		# print probability
		return -probability

	cdef computeBeta(self):
		cdef int i=0
		cdef int t=0
		cdef int j=0
		for i in range(self.num_states):
		...
		for t in reversed(range(self.len_obSeq-2)):
			for i in range(self.num_states):
				for j in range(self.num_states):
	...
	def setITrainSeq(self, np.ndarray[np.int32_t, ndim=1] trainSeq):
		self.num_obSeq=1
		self.obSeq=np.array([trainSeq])

	def setTrainSeq(self, np.ndarray[np.int32_t, ndim=1] trainSeq):
		self.num_obSeq+=1
		self.obSeq=np.vstack((self.obSeq, trainSeq))
		# print "N", self.obSeq

	def train(self):
		cdef int i=0
		for i in range(5):
			print "Re-estimating ", i, "OK"
			self.reestimate()
			

	cdef reestimate(self):
		cdef np.ndarray[np.float64_t, ndim=2] newTransition = np.empty((self.num_states, self.num_states))
		cdef np.ndarray[np.float64_t, ndim=2] newOutput = np.empty((self.num_states, self.num_symbols))
		cdef np.ndarray[np.float64_t, ndim=1] numerator = np.empty(self.num_obSeq)
		cdef np.ndarray[np.float64_t, ndim=1] denominator = np.empty(self.num_obSeq)
		cdef long double sumP=0
		cdef int t=0
		cdef int i=0
		cdef int j=0
		cdef int k=0
		cdef long double denom=0
	...
	cdef inline setObSeq(self, int [:] observationSeq):
		self.currentSeq=np.array(observationSeq)

		self.len_obSeq=observationSeq.shape[0]
		self.alpha = np.empty((self.len_obSeq, self.num_states), dtype='float64')
		self.beta = np.empty((self.len_obSeq, self.num_states), dtype='float64')
		self.scaleFactor=np.empty(self.len_obSeq, dtype='float64')


	def viterbi(self, testSeq):
		""" generated source for method viterbi """
		self.setObSeq(testSeq)
		phi = np.zeros(shape=(int(self.len_obSeq),int(self.num_states)))
		self.psi = np.zeros(shape=(int(self.len_obSeq),int(self.num_states)), dtype='int')
		self.q = np.zeros(shape=(int(self.len_obSeq)), dtype='int')
		i = 0
		while i < self.num_states:
			temp = self.pi[i]
			if temp == 0:
				temp = self.MIN_PROBABILITY
			phi[0][i] = np.log(temp) + np.log(self.output[i][self.currentSeq[0]])
			self.psi[0][i] = 0
			i += 1
		t = 1
		while t < self.len_obSeq:
			...
			while j < self.num_states:
				maxi=phi[t-1][0]+ np.log(self.transition[0][j])
				temp=0
				index=0
				i=1
				while i < self.num_states:
					temp = phi[t - 1][i] + np.log(self.transition[i][j])
					if temp > maxi:
						maxi = temp
						index = i
					i += 1
				phi[t][j] = maxi + np.log(self.output[j][self.currentSeq[t]])
				self.psi[t][j] = index
				j += 1
			t += 1
