...
class HiddenMarkov(object):
    ...
    def rescaleBeta(self, t):
        ...
        while i < self.num_states:
            ...
            i += 1
        # print "AFTER RESCALEBETA", self.beta
    ...
    def rescaleAlpha(self, t):
        ...
        while i < self.num_states:
            ...
            i += 1
        """
        self.scaleFactor[t]+=np.sum(self.alpha[t])
        self.scaleFactor[t] = 1 / self.scaleFactor[t]
        self.alpha[t]*=self.scaleFactor[t]
        # print "AFTER ALPHA", self.alpha, self.scaleFactor
    ...
    def computeAlpha(self):
        ...
        t = 0
        while t < self.len_obSeq:
            probability += np.log(self.scaleFactor[t])
            t += 1
    ...
    def __init__(self,num_states,num_symbols):
        ...
        self.transition = np.zeros(shape=(int(num_states), int(num_states)))
        self.output = np.zeros(shape=(int(num_states), int(num_symbols)))
        self.pi = np.zeros(shape=(int(num_states),))
        self.pi[0] = 1
        i = 1
        while i < num_states:
            self.pi[i] = 0
