from libc.math cimport log #check
...
cdef class HiddenMarkov:
	...
	def viterbi(self, testSeq):
		...
		cdef int t
		cdef int j
		cdef long double maxi
		cdef int index
		self.setObSeq(testSeq)
		cdef np.ndarray phi = np.zeros(shape=(int(self.len_obSeq),int(self.num_states)))
		self.psi = np.zeros(shape=(int(self.len_obSeq),int(self.num_states)),dtype=np.int32)
		self.q = np.zeros(shape=(int(self.len_obSeq)),dtype=np.int32)
		...
		while i < self.num_states:
			...
			phi[0,i] = log(temp) + log(self.output[i,self.currentSeq[0]])
			self.psi[0,i] = 0
			i += 1
		t = 1
		while t < self.len_obSeq:
			...
			while j < self.num_states:
				maxi=phi[t-1,0]+ log(self.transition[0,j])
				temp=0
				index=0
				i=1
				while i < self.num_states:
					temp = phi[t - 1,i] + log(self.transition[i,j])
					if temp > maxi:
						maxi = temp
						index = i
					i += 1
				phi[t,j] = maxi + log(self.output[j,self.currentSeq[t]])
				self.psi[t,j] = index
				j += 1
			t += 1
	...
	cdef inline rescaleBeta(self, int t):
		...
		# print "AFTER RESCALEBETA", np.array(self.beta)
	...
	cdef inline rescaleAlpha(self,int t):
		...
		# print "AFTER ALPHA", np.array(self.alpha), np.array(self.scaleFactor)
	...
	cdef double computeAlpha(self):
		""" generated source for method computeAlpha """
		cdef double probability = 0
		cdef int t = 0
		cdef int i
		cdef int j
		cdef long double sumi

		t=self.len_obSeq
		# self.scaleFactor[:t] = 0
		self.scaleFactor=np.zeros(shape=(self.len_obSeq,))
	...
	def setTrainSeq(self, trainSeq):
		...
		# print trainSeq[1]
		# self.obSeq = np.zeros(shape=(int(self.num_obSeq), ))
		# k = 0
	...
	def setObSeq(self,  observationSeq):
		...
		self.alpha = np.zeros(shape=(int(self.len_obSeq), int(self.num_states)), dtype='float64')
		self.beta = np.zeros(shape=(int(self.len_obSeq), int(self.num_states)), dtype='float64')
		self.scaleFactor = np.zeros(shape=(int(self.len_obSeq),), dtype='float64')
	...
	def reestimate(self):
		...
		cdef int k
		cdef int t
		cdef np.ndarray newTransition = np.zeros([int(self.num_states),int(self.num_states)])
		cdef np.ndarray newOutput = np.zeros([int(self.num_states),int(self.num_symbols)])
		cdef np.ndarray numerator = np.zeros([int(self.num_obSeq),])
		cdef np.ndarray denominator = np.zeros([int(self.num_obSeq),])
		cdef double sumP = 0
		i = 0
		while i < self.num_states:
			j=0
			while j < self.num_states:
				if j < i or j > i + self.delta:
					...
					while k < self.num_obSeq:
						...
						self.setObSeq(np.array(self.obSeq[k], dtype='int'))
						...
						while t < self.len_obSeq - 1:
							...
							# print "NUMDEN", numerator[k], denominator[k]
							t += 1
						k += 1
					denom=0
					k=0
	...
	def __init__(self,int num_states,int num_symbols):
		...
		self.transition = np.zeros([num_states, num_states])
		self.output = np.zeros([num_states, num_symbols])
	...
	def save(self, modelName):
		...
		model = HMMModel()
		model.output=np.array(self.output)
		model.pi=np.array(self.pi)
		model.transition=np.array(self.transition)
	...
	def initFromName(self, name):
		...
		self.output=np.array(newHMMM.output)
		self.transition=np.array(newHMMM.transition)
		self.pi=np.array(newHMMM.pi)
		self.num_states=len(self.output)
		self.num_symbols=len(self.output[0])
		# print "LOADED", np.array(self.output), np.array(self.transition), np.array(self.pi), self.num_states, self.num_symbols
