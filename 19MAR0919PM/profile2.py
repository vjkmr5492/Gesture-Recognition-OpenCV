...
while True:
	...
	tr_3=cv2.calcBackProject([frame],[0,1],rh3,[0,256,0,256],1)


	roi_4=orig[int(cols/2.3):int(cols/2.3)+SQ_SIZE, int(rows/4):int(rows/4+SQ_SIZE)]
	rh4 = cv2.calcHist([roi_4],[0, 1], None, [256, 256], [0, 256, 0, 256] )
	if(reset):
	...
	tr_4=cv2.calcBackProject([frame],[0,1],rh4,[0,256,0,256],1)


	roi_5=orig[int(cols/2.5):int(cols/2.5)+SQ_SIZE, int(rows/2):int(rows/2+SQ_SIZE)]
	rh5 = cv2.calcHist([roi_5],[0, 1], None, [256, 256], [0, 256, 0, 256] )
	if(reset):
	...
	tr_5=cv2.calcBackProject([frame],[0,1],rh5,[0,256,0,256],1)


	roi_6=orig[int(cols/8):int(cols/8)+SQ_SIZE, int(rows/2.7):int(rows/2.7+SQ_SIZE)]
	rh6 = cv2.calcHist([roi_6],[0, 1], None, [256, 256], [0, 256, 0, 256] )
	if(reset):
	...
	tr_6=cv2.calcBackProject([frame],[0,1],rh6,[0,256,0,256],1)
	...
	ret,thresh = cv2.threshold(tr_1,50,255,0)
